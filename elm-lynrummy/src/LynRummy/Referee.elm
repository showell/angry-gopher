module LynRummy.Referee exposing
    ( RefereeError
    , RefereeMove
    , RefereeStage(..)
    , computeBoardAfter
...
import LynRummy.BoardGeometry exposing (BoardBounds, GeometryError, validateBoardGeometry)
...
stackDebugStr s =
    s.boardCards
        |> List.map (.card >> LynRummy.Card.cardStr)
        |> String.join ","
