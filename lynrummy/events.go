...
// BoardFingerprint returns a compact string representation of the
// board for debugging. Same format as Angry Cat's
// board_fingerprint.
func BoardFingerprint(board []CardStack) string {
	...
	for i, s := range board {
		...
		result += fmt.Sprintf("(%d,%d) [%s]", s.Loc.Left, s.Loc.Top, cards)
	}
	return result
}
