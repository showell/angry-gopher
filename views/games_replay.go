// Game replay view — canvas-based scrubber with retroactive
// trick detection. Separated from views/games.go so the CRUD
// parts (list, detail, forms) can be expressed in the DSL
// while this non-CRUD view stays as hand-written Go.
//
// Registered at /gopher/game-replay?id=N. The detail view links
// here; no top-level nav entry (NavLabel empty).
...
func HandleGameReplay(w http.ResponseWriter, r *http.Request) {
	...
	if gameID <= 0 {
		http.Error(w, "Missing or invalid game id", http.StatusBadRequest)
		return
	}
	renderGameReplay(w, userID, gameID)
}

func init() {
	registerGeneratedPage(PageDef{
		Path:     "/gopher/game-replay",
		NavLabel: "",
		Title:    "Game Replay",
		Subtitle: "Step through a recorded LynRummy game.",
		Handler:  HandleGameReplay,
	})
}
