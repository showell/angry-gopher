...
// HandlePuzzles dispatches /gopher/puzzles/*. As of 2026-04-28
// (LEAN_PASS phase 2) the action / annotate endpoints write to
// the unified games/lynrummy/data/ tree — Go is dumb file
// storage, no DB. The endpoints stay where they are (Elm still
// posts to them) until the Elm client switches to the unified
// /lynrummy-elm/sessions/<id>/... URL space.
...
// puzzlesActionsShim is the back-compat handler for
// POST /gopher/puzzles/actions?session=<id>&puzzle=<name>.
// Auto-allocates a per-session seq and writes the body verbatim
// to data/lynrummy-elm/sessions/<id>/actions/<seq>.json. The
// puzzle_name is preserved by the body (Elm includes it via
// query param today; we record it alongside).
func puzzlesActionsShim(w http.ResponseWriter, r *http.Request) {
	...
	rel := "actions/" + strconv.FormatInt(nextSeq, 10) + ".json"
