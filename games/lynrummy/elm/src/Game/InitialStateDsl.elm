module Game.InitialStateDsl exposing
    ( formatGameState
    , parseGameState
    )

{-| DSL encoder + parser for a full `GameState` — what
`Main.State.encodeGameState` and `Main.Wire.initialStateDecoder`
...
(`Game.HandDsl`, `Game.BoardDsl`); this module dispatches on
...
import Game.HandDsl as HandDsl
