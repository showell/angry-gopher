module Lib.Rules.Card exposing
    ( Card
    , CardColor(..)
    , CardValue(..)
    , OriginDeck(..)
...
canonical. Intentional Elm divergences:
  ...
  - PRNG (`mulberry32`) lives in `Lib.Random` — this module
...
right one for the right question:
  ...
  - **`isPairOfDups`** — checks `value` and `suit`, ignores
    ...
    (`getStackType` in `Lib.Rules.StackType`) uses this to
    detect dup sets.
...
-}
buildFullDoubleDeck : Lib.Random.Seed -> ( List Card, Lib.Random.Seed )
buildFullDoubleDeck seed =
    ...
    in
    Lib.Random.shuffle seed allCards
