module Lib.HandLayout exposing
    ( handCardDomId
    , handLeft
    , handTop
    , positionAt
...
import Lib.Rules.Card exposing (Card, OriginDeck(..))
...
handCardDomId card =
    "hand-card-v"
        ++ String.fromInt (Lib.Rules.Card.cardValueToInt card.value)
        ++ "-s"
        ++ String.fromInt (Lib.Rules.Card.suitToInt card.suit)
        ++ "-d"
