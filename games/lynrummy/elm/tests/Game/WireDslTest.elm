module Game.WireDslTest exposing (suite)

{-| Round-trip + literal-shape tests for the wire DSL grammar.
Verifies each per-event encoder in `Game.GameEvent` produces a
line that `Game.WireAction.parseDsl` decodes back to the same
event (plus seq).
