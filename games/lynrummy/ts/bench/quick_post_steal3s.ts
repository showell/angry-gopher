// Subproblem: applied "steal 3S from [AS 2S 3S] onto [4H 5S]"
// manually as the first step (instead of shift). Now [AS 2S] is
// in trouble. Test whether the BFS can resolve from here without
// shift.
...
const raw: RawBuckets = {
  helper: [
    P("3C","4C'","5C'"),
    // [AS 2S 3S] gone — its 3S went to focus, [AS 2S] spawned to trouble.
    P("3D","4C","5H","6S","7D'"),
    P("7S","7D","7C","7H"),
    P("KH","AC","2H'"),
    P("KS","AD'","2C","3D'"),     // KS still here — available to peel.
    P("TD","JD","QD","KD"),
  ],
  // initialLineage seeds in input order; matching real BFS's
  // post-steal-3S lineage = [[AC'], [AD], [AS 2S]] with focus [AC'].
