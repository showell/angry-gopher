...
const DSL_PATHS: readonly string[] = [
  path.join(DSL_DIR, "replay_walkthroughs.dsl"),
  path.join(DSL_DIR, "puzzle_walkthroughs.dsl"),
];
...
function main(): void {
  ...
  for (const dslPath of DSL_PATHS) {
    if (!fs.existsSync(dslPath)) {
      // puzzle_walkthroughs.dsl is generated by replay_puzzles.ts —
      // missing is a soft failure (print + skip), not a hard one.
      // The mined replay_walkthroughs.dsl is checked in and required.
      if (path.basename(dslPath) === "replay_walkthroughs.dsl") {
        console.error(`no DSL at ${dslPath}`);
        process.exit(1);
      }
      console.log(`(skipping missing ${path.basename(dslPath)})`);
      continue;
    }
    const contents = fs.readFileSync(dslPath, "utf8");
    walkthroughs.push(...parseDsl(contents));
  }
