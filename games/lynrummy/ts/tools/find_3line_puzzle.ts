// find_3line_puzzle.ts — one-off discovery script. Throwaway.
//
...
// up to WANT). 3-line plays are the second-easiest tier; we bias toward
// small boards so the puzzle reads clearly.
//
...
function main(): void {
  ...
  if (emitDsl) {
    ...
    console.log(`# ${TARGET_PLAN_LENGTH} verb-level moves — the second-easiest tier. Curated for`);
    console.log(`# variety (one puzzle per verb-multiset), then ordered by total cards on`);
