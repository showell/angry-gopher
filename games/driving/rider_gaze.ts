...
// (Which legs distract him — the first PIG_NOVELTY_COUNT pig appearances — is decided in world.ts and read off
// seg.pigsDistract below; that flag also sizes the herd, so the big herd and the gawk can't disagree.)
...
// How far the designated pig is still AHEAD of the rider, but only while he's in the looking window: a
// DISTRACTION leg (seg.pigsDistract — set in world.ts), eyes not already committed to a corner, the pig within
...
function pigAhead(state: RiderState, seg: RoadSegment): number | null {
  if (!seg.pigsDistract) return null;                        // not a distraction leg (no pigs, or novelty worn off)
...
// Once he's NOTICED the pigs on a distraction leg (the designated pig has come within GAZE_LOOK_DIST ahead),
// he's committed to the slow gawk for the REST of the segment: he eases to the gawk speed and never
// re-accelerates until he leaves the segment (the crossing resets him). Sticky and purely positional — he
// can't un-notice — so no counter in his state. Distinct from pigAhead (the HEAD-turn), which also ends once
// the pig is beside him; this is just the MOTION hold, so it ignores the eyes-on-road gate and persists into the turn.
export function gawkEngaged(state: RiderState, seg: RoadSegment): boolean {
  if (!seg.pigsDistract) return false;
