...
// baked from a vendored Twemoji SVG (emoji_svg/*.svg, flat fill-only vector art, CC-BY) into a flat list
...
// segments, turn each subpath into one solid polygon of the element's fill colour. (Twemoji uses no
...
interface RawPoly { color: number; pts: Pt[] } // color 0xRRGGBB, pts in SVG coords (y down)
...
// (absolute + relative); no A (verified absent in Twemoji). Implicit command repetition is handled.
...
function parseColor(s: string | undefined): number {
  if (!s || s === 'none') return -1; // sentinel: skip
...
function svgToPolys(svg: string): RawPoly[] {
  const out: RawPoly[] = [];
...
function normalize(polys: RawPoly[]): RawPoly[] {
...
let zig = `//! emoji_frames — GENERATED by ops/bake_emoji from emoji_svg/*.svg (vendored Twemoji, CC-BY). DO NOT EDIT BY HAND.
//!
...
for (let ci = 0; ci < baked.length; ci++) {
  const ox = ci * CELL + CELL / 2, oy = ci ? 0 : 0;
