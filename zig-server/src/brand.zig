...
//! allowlist (port of server/platform/images.go). Not to be confused with
//! images.zig (/chat/images — the per-user chat image transcript); these are the
//! binary's own brand images, embedded at build time (build.zig). TOTALLY_PUBLIC.
//!
//! The allowlist IS the path guard — no directory walk, no traversal surface.
//! Adding an image is one build.zig embed + one line in assetFor here, the same
//! two-edit discipline Go's imageAllowlist + //go:embed pairing enforces.
...
/// assetFor maps an allowlisted filename to its embedded bytes, or null. Mirrors
/// Go's imageAllowlist lookup + ReadAsset.
fn assetFor(file: []const u8) ?[]const u8 {
    if (std.mem.eql(u8, file, "cat_professor.webp")) return cat_professor;
    return null;
}

/// contentType derives the MIME type from the extension (Go's suffix switch).
/// Only the extensions the binary actually hosts are wired.
