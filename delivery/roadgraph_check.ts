...
import { buildSubstrate, RING, SERVICE, SPEED, edges } from "./roadgraph.ts";
...
console.log(`speed factors:`, SPEED, `  RING=${RING}  SERVICE=${SERVICE}\n`);
...
// 5) Coarse cost floor: every active neighborhood costs at least one RING + its
//    orders' SERVICE, plus a round trip from the FC. Just a smell test, not the solver.
let floor = 0;
for (const [n, d] of demand) floor += RING + d * SERVICE + 2 * sub.time("FC", n);
