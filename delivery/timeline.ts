...
//   arc     — a stretch of ring road between two doorsteps (or the gates)
//   enter   — the pull-in pause at a cul-de-sac mouth (ENTER minutes)
...
import { ENTER, SERVICE, localMinutes } from "./roadgraph.ts";
...
/**
 * Lay the ring road of one stop into legs: drive the arc, stopping at each
 * ordered house *where the path actually passes it*, with a service pause
 * there. `arcMin` (the ring-driving minutes, ENTER excluded) is spread across
 * the arc by length, so the leg times still sum to the cost model's local time.
 *
 * `served` dedupes service across the whole route: if a neighborhood lies on
 * the shortest path to a later stop, it shows up twice in the node list, and
 * the cost model charges its ring driving (and ENTER) both times — but each
 * order's SERVICE only once. So we service a house at its first occurrence only.
 */
function ringLegs(name: string, pts: Pt[], arcMin: number, houses: number[], served: Set<string>): Leg[] {
  const cum = len(pts);
  const total = cum[cum.length - 1] || 1;
  const n = NEIGHBORHOODS.find((m) => m.name === name)!;
  const angles = houseAngles(name, houses);

  // Each not-yet-served house, at the cumulative length where the truck rolls
  // past its door.
  const stops = houses
    .map((h, i) => {
    ...
    .filter((s) => !served.has(s.house))
    .sort((a, b) => a.pos - b.pos);
  for (const s of stops) served.add(s.house);
...
export function buildItinerary(sub: Substrate, route: Route): Itinerary {
  ...
  const served = new Set<string>(); // each house gets exactly one service stop
  const legs: Leg[] = [];
  for (let i = 1; i < nodes.length; i++) {
    ...
    if (node !== "FC" && i < nodes.length - 1) {
      ...
      if (delivering) legs.push({ kind: "enter", at: pts[0], dur: ENTER });
      for (const leg of ringLegs(node, pts, arcMin, houses, served)) legs.push(leg);
    }
  }
